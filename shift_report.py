...
def analyze_tasks(groups):
    ...
    system = (
        "You analyze Telegram support-group transcripts for Charlotte, a fuel-card "
        "customer service agent at Octane/TSS (trucking fuel cards). Messages from "
        "'ME' are Charlotte's own; other staff members and customers appear under "
        "their names. Identify each DISTINCT customer service request in the "
        "transcript — e.g. card activation, money code, card unlock, payment/billing "
        "issue, transaction check, support question. ONLY count customer service "
        "requests: skip sales inquiries, internal/ops chatter, chit-chat, greetings, "
        "and bot/system notifications. Several messages about the same request are "
        "ONE task; unrelated requests in the same chat are separate tasks. "
        "For each task set handled_by: 'me' if Charlotte replied to or resolved it "
        "within this transcript; 'other' if a different team member answered it and "
        "Charlotte did not; 'nobody' if no one addressed it. The customer service "
        "team has many members besides Charlotte, working in different roles "
        "(activations, billing, money codes, etc.) — Layla, Mason and Dustin are "
        "some examples, but there are others. A reply from ANY team/staff member "
        "means the task is handled ('other'), NOT unanswered. Staff are usually "
        "recognizable because they answer requests in a service role rather than "
        "asking for help. "
        "The transcript covers "
        "only Charlotte's shift window — judge strictly by what is inside it. "
        "Give each task a short label of 2-5 words. If there are no real tasks, "
